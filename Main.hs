...
ageOn planet ageInSeconds = 
  ...
  in case planet of
    "Mercury" -> Right $ ageInSeconds / (earthYearInSeconds * 0.2408467)
    "Venus" -> Right $ ageInSeconds / (earthYearInSeconds * 0.61519726)
    "Earth" -> Right $ ageInSeconds / earthYearInSeconds
    "Mars" -> Right $ ageInSeconds / (earthYearInSeconds * 1.8808158)
    "Jupiter" -> Right $ ageInSeconds / (earthYearInSeconds * 11.862615)
    "Saturn" -> Right $ ageInSeconds / (earthYearInSeconds * 29.447498)
    "Uranus" -> Right $ ageInSeconds / (earthYearInSeconds * 84.016846)
    "Neptune" -> Right $ ageInSeconds / (earthYearInSeconds * 164.79132)
    "Pluto" -> Left "Pluto is not a planet"
    _ -> Left "There is no such planet"

isLeapYear :: Int -> Either String Bool
isLeapYear year
  | year < 0 = Left "Year cannot be negative"
  | mod year 400 == 0 = Right True
  | mod year 100 == 0 = Right False
  | mod year 4 == 0 = Right True
